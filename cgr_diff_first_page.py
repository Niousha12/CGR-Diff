...
class CGRApp(ctk.CTk):
    def __init__(self):
        ...
        # analysis state (used by the cgr_analysis right panel)
        self.kmer_freq = None
        self.kmer_hist_canvas = None
    ...
    def _toggle_theme(self):
        ...
        ctk.set_appearance_mode(ThemeManager.mode)
        self._refresh_ui(full_rebuild=True)
    ...
    def _build_cgr_analysis(self, parent):
        ...
        # ---------- Design right panel ----------
        right = ctk.CTkFrame(parent, fg_color=self.colors["BG_PANEL"], border_width=1,
                             border_color=self.colors["BORDER"], corner_radius=10, )
        right.grid(row=0, column=1, padx=(5, 5), pady=(5, 5), sticky="nsew")

        right.grid_columnconfigure(0, weight=1)

        # Top two rows share the same height (uniform group)
        right.grid_rowconfigure(0, weight=1, uniform="top_hist")
        right.grid_rowconfigure(1, weight=1, uniform="top_hist")
        # Bottom row can be shorter
        right.grid_rowconfigure(2, weight=0)

        # 1) Top frame – real k-mer histogram
        self.cgr_hist_frame1 = ctk.CTkFrame(right, fg_color=self.colors["CARD"], corner_radius=8, )
        self.cgr_hist_frame1.grid(row=0, column=0, padx=10, pady=(10, 5), sticky="nsew")

        # 2) Second frame – placeholder for another histogram
        self.cgr_hist_frame2 = ctk.CTkFrame(right, fg_color=self.colors["CARD"], corner_radius=8, )
        self.cgr_hist_frame2.grid(row=1, column=0, padx=10, pady=5, sticky="nsew")

        # 3) Bottom row with two placeholders stays as you had it
        bottom = ctk.CTkFrame(right, fg_color="transparent")
        bottom.grid(row=2, column=0, padx=10, pady=(5, 10), sticky="nsew")
        bottom.grid_columnconfigure((0, 1), weight=1)
        bottom.grid_rowconfigure(0, weight=1)

        self.cgr_bottom_left = ctk.CTkFrame(bottom, fg_color=self.colors["CARD"], corner_radius=8)
        self.cgr_bottom_left.grid(row=0, column=0, padx=(0, 5), sticky="nsew")
        lbl_bl = ctk.CTkLabel(self.cgr_bottom_left, text="Bottom-left placeholder", font=FONT_SM,
                              text_color=self.colors["TEXT_MUTED"], )
        lbl_bl.pack(expand=True)

        self.cgr_bottom_right = ctk.CTkFrame(bottom, fg_color=self.colors["CARD"], corner_radius=8)
        self.cgr_bottom_right.grid(row=0, column=1, padx=(5, 0), sticky="nsew")
        lbl_br = ctk.CTkLabel(self.cgr_bottom_right, text="Bottom-right placeholder", font=FONT_SM,
                              text_color=self.colors["TEXT_MUTED"], )
        lbl_br.pack(expand=True)
    ...
    def _draw_kmer_histogram(self):
        """Render a bar chart of the current k-mer frequencies into the top-right frame."""
        if not getattr(self, "cgr_hist_frame1", None):
            return
        if not self.kmer_freq:
            return

        # Remove previous plot if it exists
        if getattr(self, "kmer_hist_canvas", None) is not None:
            try:
                self.kmer_hist_canvas.get_tk_widget().destroy()
            except Exception:
                pass
            self.kmer_hist_canvas = None

        # --- Prepare data: take top-N k-mers by frequency for readability ---
        items = sorted(self.kmer_freq.items(), key=lambda kv: kv[1], reverse=True)

        kmers = [k for k, _ in items]
        ...
        fig = Figure(figsize=(6, 3), dpi=180, facecolor=card_color, constrained_layout=True, )
        ax = fig.add_subplot(111)
        ax.set_facecolor(card_color)

        # Bar plot
        ax.bar(range(len(kmers)), counts)

        # Axes styling
        ax.set_xticks(range(len(kmers)))
        ax.set_xticklabels(kmers, rotation=90, ha="right", fontsize=5, color=text_color, )
        ax.set_yticklabels(ax.get_yticks(), fontsize=5, color=text_color, )
        ax.set_ylabel("Count", color=text_color, fontsize=5, rotation=90)
        # ax.set_xlabel("k-mer", color=text_color, fontsize=5)

        k_val = len(kmers[0])
        title = f"{k_val}-mer frequency"
        ax.set_title(title, color=text_color, fontsize=8)

        # Only left and bottom axes (no surrounding box)
        ...
        canvas = FigureCanvasTkAgg(fig, master=self.cgr_hist_frame1)
        canvas.draw()
        widget = canvas.get_tk_widget()
        widget.pack(fill="both", expand=True, padx=10, pady=(0, 10))

        self.kmer_hist_canvas = canvas
    ...
    @staticmethod
    def _analyze_kmer_frequency(seq, k):
        kmer_counts = {}
